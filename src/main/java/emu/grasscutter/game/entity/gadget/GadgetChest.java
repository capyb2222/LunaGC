...
import emu.grasscutter.net.proto.BossChestInfo;
    ...
    public boolean onInteract(Player player, GadgetInteractReq req) {
        ...
        if (Grasscutter.getConfig().server.game.enableScriptInBigWorld) {
            ...
            if (chest.boss_chest != null && chest.drop_tag != null) {
                ...
                if (req.getOpType() == InterOpType.INTER_OP_START) {
                    // Two steps
                    player.sendPacket(
                            new PacketGadgetInteractRsp(
                                    getGadget(),
                                    InteractType.INTERACT_OPEN_CHEST,
                                    InterOpType.INTER_OP_START));
                    return false;
                }
        ...
        if (req.getOpType() == InterOpType.INTER_OP_START && handler.isTwoStep()) {
            player.sendPacket(
                    new PacketGadgetInteractRsp(
                            getGadget(), InteractType.INTERACT_OPEN_CHEST, InterOpType.INTER_OP_START));
            return false;
        } else {
            boolean success;

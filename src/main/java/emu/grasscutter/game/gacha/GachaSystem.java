...
import emu.grasscutter.net.proto.GachaTransferItem;
import emu.grasscutter.net.proto.GetGachaInfoRsp;
...
public class GachaSystem extends BaseGameSystem {
    ...
    // Set by doPull/doRarePull when Capturing Radiance turns a lost 50/50 into a featured win, so the
    // caller can flag the resulting GachaItem (is_flash_card) and trigger the radiance animation.
    // Safe as a plain field because doPulls/doPull/doRarePull are all synchronized on this instance.
    private boolean lastPullCapturedRadiance = false;
    ...
    private synchronized int doRarePull(
        ...
        // Capturing Radiance only applies to a limited character banner's 5-star 50/50 (not weapon/
        // chronicle epitomized banners, nor standard/beginner banners which have no featured 5-star).
        boolean radianceEligible =
                (rarity == 5) && !banner.hasEpitomized() && (featured.length > 0);
        ...
        if (epitomized && pityEpitomized) { // Auto pick item when epitomized points reached
            ...
            } else if (radianceEligible
                    && this.randomRange(1, 100) <= banner.getCapturingRadianceChance()) {
                // Lost the 50/50, but Capturing Radiance converts it into a featured win.
                gachaInfo.setFailedFeaturedItemPulls(rarity, 0);
                itemId = getRandom(featured);
                this.lastPullCapturedRadiance = true; // Plays the radiance animation client-side
            } else {
    ...
    private synchronized int doPull(
        ...
        this.lastPullCapturedRadiance = false; // Reset per pull; doRarePull may set it
    ...
    public synchronized void doPulls(Player player, int scheduleId, int times) {
        ...
        for (int i = 0; i < times; i++) {
            ...
            boolean capturedRadiance = this.lastPullCapturedRadiance;
            ...
            if (capturedRadiance) {
                gachaItem.setIsFlashCard(true); // Capturing Radiance -> special wish animation
            }

...
import emu.grasscutter.net.proto.GachaInfo;
import emu.grasscutter.net.proto.GachaUpInfo;
...
public class GachaBanner {
    ...
    // Capturing Radiance: % chance that a lost 5-star 50/50 on a character banner is converted into a
    // featured win. ~10 approximates the official ~55% overall featured rate.
    // Set to 0 to disable, or 100 to always capture. Configurable per-banner in Banners.json.
    @Getter private int capturingRadianceChance = 10;
    ...
    public GachaInfo toProto(Player player) {
        ...
        GachaInfo.Builder info =
                GachaInfo.newBuilder()
                        ...
                        .setKIPDIEOHELN(this.costItemId) // cost_item_id
                        .setIICCKKMBIIN(this.costItemId10) // ten_cost_item_id
                        .setGachaPrefabPath(this.getPrefabPath())
                        .setGachaPreviewPrefabPath(this.getPreviewPrefabPath())
                        .setDGOIEHCEBFM(record) // gacha_record_url
                        .setOEIKDNCFFKG(record) // gacha_record_url_oversea
                        .setLCNBNGKMEFE(details) // gacha_prob_url
                        .setLOBIEKALOEM(details) // gacha_prob_url_oversea
        ...
        // Advertise the Capturing Radiance feature on limited character banners. The per-item
        // is_flash_card flag alone does not trigger the animation; the banner must enable it.
        // KHANMGINCMP ("new_bool") / PGNMPDKFCKE ("is_new_wish") per 6.6.0_gacha_nt.txt.
        if (!hasEpitomized() && this.getRateUpItems5().length > 0) {
            info.setKHANMGINCMP(true); // new_bool
            info.setPGNMPDKFCKE(true); // is_new_wish
        }
        ...
        if (this.getRateUpItems5().length > 0) {
            ...
            for (int id : getRateUpItems5()) {
                ...
                if (hasEpitomized()) info.addAFBKFLFOGMF(id);
            }

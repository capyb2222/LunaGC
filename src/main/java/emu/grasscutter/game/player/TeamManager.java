...
import emu.grasscutter.net.proto.EnterType;
...
public final class TeamManager extends BasePlayerDataManager {
    ...
    public AbilityControlBlock getAbilityControlBlock() {
        AbilityControlBlock.Builder abilityControlBlock =
            AbilityControlBlock.newBuilder();
        int embryoId = 0;
        ...
        this.getActiveTeam()
            .forEach(
                x ->
                    this.getPlayer()
                        .getScene()
                        .removeEntity(x, VisionType.VISION_REMOVE));
    ...
    public void removeTrialAvatarTeam(List<Integer> trialAvatarIds) {
        ...
        this.getActiveTeam()
            .forEach(
                avatarEntity ->
                    scene.removeEntity(
                        avatarEntity, VisionType.VISION_REMOVE));
    ...
    public synchronized void changeAvatar(long guid) {
        ...
        oldEntity.setMotionState(MotionState.MOTION_STATE_MOTION_STANDBY);
    ...
    public void onAvatarDie(long dieGuid) {
        ...
        if (dieType == PlayerDieType.PLAYER_DIE_PLAYER_DIE_DRAWN) {

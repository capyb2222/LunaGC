...
public class HomeWorldMPSystem extends BaseGameSystem {
    ...
    public void enterHome(Player requester, Player owner, int teleportPoint, boolean toSafe) {
        ...
        if (owner.getRealmList() == null) {
            ...
            requester.sendPacket(
                    new PacketTryEnterHomeRsp(
                            Retcode.RET_HOME_NOT_FOUND_IN_MEM_VALUE, owner.getUid()));
            return;
        }
        ...
        if (event.isCanceled()) {
            requester.sendPacket(
                    new PacketTryEnterHomeRsp(
                            Retcode.RET_HOME_OWNER_REFUSE_TO_ENTER_HOME_VALUE, owner.getUid()));
            return;
        }
        ...
        if (owner.isInEditMode()) {
            requester.sendPacket(
                    new PacketTryEnterHomeRsp(
                            Retcode.RET_HOME_CANT_ENTER_BY_IN_EDIT_MODE_VALUE, owner.getUid()));
            return;
        }
    ...
    public boolean leaveCoop(Player player, int prevScene, Position pos) {
        ...
        player
                ...
                .sendPacketToHostIfOnline(
                        new PacketOtherPlayerEnterOrLeaveHomeNotify(
                                player,
                                OtherPlayerEnterHomeNotify.Reason.LEAVE));
        ...
        player.sendPacket(
                new PacketPlayerQuitFromHomeNotify(
                        PlayerQuitFromHomeNotify.QuitReason
                                .BACK_TO_MY_WORLD));
        player.sendPacket(
                new PacketPlayerEnterSceneNotify(
                        player,
                        EnterType.ENTER_BACK,
    ...
    public boolean kickPlayerFromHome(Player owner, int targetUid) {
        ...
        victim.sendPacket(
                new PacketPlayerQuitFromHomeNotify(
                        PlayerQuitFromHomeNotify.QuitReason.KICK_BY_HOST));
        victim.sendPacket(
                new PacketPlayerEnterSceneNotify(
                        victim,
                        EnterType.ENTER_BACK,

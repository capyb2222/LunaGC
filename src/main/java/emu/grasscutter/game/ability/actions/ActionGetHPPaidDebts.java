...
public final class ActionGetHPPaidDebts extends AbilityActionHandler {
    ...
    public boolean execute(Ability ability, AbilityModifier.AbilityModifierAction action, ByteString abilityData, GameEntity target) {
        ...
        if (target instanceof EntityAvatar) {
            ...
            target.getWorld().broadcastPacket(new PacketEntityFightPropChangeReasonNotify(target, FightProperty.FIGHT_PROP_CUR_HP_PAID_DEBTS, paiddebt, PropChangeReason.PROP_CHANGE_REASON_PROP_CHANGE_ABILITY, _ChangeHpDebtsReason._CHANGE_HP_DEBTS_REASON_CHANGE_HP_DEBTS_PAY));
        } else {

    ...
    if (target.isConvertToHpDebt() && ability.getOwner() != target) {
        ...
        if (changeDebt > 0) {
            target.getWorld().broadcastPacket(new PacketEntityFightPropChangeReasonNotify(
                ...
                _ChangeHpDebtsReason._CHANGE_HP_DEBTS_REASON_CHANGE_HP_DEBTS_ADD_ABILITY
            ));
        }
        ...
        if (curHp < maxHp && furina != null) {
                ...
                furina.addEnergy(energyToAdd, PropChangeReason.PROP_CHANGE_REASON_PROP_CHANGE_ABILITY);
            }

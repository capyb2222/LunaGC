...
public final class ActionAddHPDebts extends AbilityActionHandler {
    ...
    public boolean execute(Ability ability, AbilityModifier.AbilityModifierAction action, ByteString abilityData, GameEntity target) {
        ...
        float maxValue = action.maxValue.get(ability) * target.getFightProperty(FightProperty.FIGHT_PROP_MAX_HP);
            ...
            if (changeDebt != 0) {
                if (newDebt == 0) {
                    target.getWorld().broadcastPacket(new PacketEntityFightPropChangeReasonNotify(target, FightProperty.FIGHT_PROP_CUR_HP_DEBTS, changeDebt, PropChangeReason.PROP_CHANGE_REASON_PROP_CHANGE_ABILITY, _ChangeHpDebtsReason._CHANGE_HP_DEBTS_REASON_CHANGE_HP_DEBTS_PAY_FINISH));
                } else if (changeDebt > 0) {
                    target.getWorld().broadcastPacket(new PacketEntityFightPropChangeReasonNotify(target, FightProperty.FIGHT_PROP_CUR_HP_DEBTS, changeDebt, PropChangeReason.PROP_CHANGE_REASON_PROP_CHANGE_ABILITY, _ChangeHpDebtsReason._CHANGE_HP_DEBTS_REASON_CHANGE_HP_DEBTS_ADD_ABILITY));
                } else if (changeDebt < 0) {
                    target.getWorld().broadcastPacket(new PacketEntityFightPropChangeReasonNotify(target, FightProperty.FIGHT_PROP_CUR_HP_DEBTS, changeDebt, PropChangeReason.PROP_CHANGE_REASON_PROP_CHANGE_ABILITY, _ChangeHpDebtsReason._CHANGE_HP_DEBTS_REASON_CHANGE_HP_DEBTS_PAY));
                }
            }
        return true;
    }

...
import emu.grasscutter.net.proto.GetCompoundDataReq;
...
public class CookingCompoundManager extends BasePlayerManager {
        ...
        for (var item : player.getActiveCookCompounds().values()) {
            var data =
                    CompoundQueueData.newBuilder()
                            ...
                            .setNOJHLEGJNNL(item.getOutputCount(currentTime))
                            .setMANNMCFAFPN(item.getOutputTime(currentTime))
                            .setNDBBPDNNMEP(item.getWaitCount(currentTime))
                            .build();
    ...
    public synchronized void handlePlayerCompoundMaterialReq(PlayerCompoundMaterialReq req) {
        ...
        var data =
                CompoundQueueData.newBuilder()
                        .setCompoundId(id)
                        .setNOJHLEGJNNL(c.getOutputCount(currentTime))
                        .setMANNMCFAFPN(c.getOutputTime(currentTime))
                        .setNDBBPDNNMEP(c.getWaitCount(currentTime))
                        .build();

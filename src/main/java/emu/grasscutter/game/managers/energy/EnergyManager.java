...
import emu.grasscutter.net.proto.AbilityActionGenerateElemBall;
import emu.grasscutter.net.proto.AbilityIdentifier;
...
public class EnergyManager extends BasePlayerManager {
    ...
    private void generateEnergyForNormalAndCharged(EntityAvatar avatar) {
        ...
        if (roll < currentProbability) {
            avatar.addEnergy(1.0f, PropChangeReason.PROP_CHANGE_REASON_PROP_CHANGE_ABILITY, true);
            this.avatarNormalProbabilities.put(avatar, weaponType.getEnergyGainInitialProbability());
        }
    ...
    public void handleAttackHit(EvtBeingHitInfo hitInfo) {
        ...
        GameEntity targetEntity = this.player.getScene().getEntityById(attackRes.getONMNILILOAJ());
    ...
    private void handleBurstCast(Avatar avatar, int skillId) {
        ...
        if(avatar.getAvatarId()==10000106){
            ...
            if(skillData.getSpecialEnergyMin() > 0){
                avatar.getAsEntity().clearEnergy(ChangeEnergyReason.CHANGE_ENERGY_REASON_CHANGE_ENERGY_SKILL_START);
            }
        }
    ...
    public void setEnergyUsage(boolean energyUsage) {
        ...
        if (!energyUsage) { // Refill team energy if usage is disabled
            this.refillTeamEnergy(PropChangeReason.PROP_CHANGE_REASON_PROP_CHANGE_GM, true);
        }
    }
}

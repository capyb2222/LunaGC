...
import emu.grasscutter.net.proto.BargainOfferPriceReq;
...
public final class HandlerBargainOfferPriceReq extends PacketHandler {
    ...
    public void handle(GameSession session, byte[] header, byte[] payload) throws Exception {
        ...
        switch (result) {
            case BARGAIN_RESULT_TYPE_BARGAIN_COMPLETE_SUCC -> questManager.queueEvent(
                    QuestContent.QUEST_CONTENT_BARGAIN_SUCC, bargainId, 0);
            case BARGAIN_RESULT_TYPE_BARGAIN_SINGLE_FAIL -> questManager.queueEvent(
                    QuestContent.QUEST_CONTENT_ITEM_LESS_THAN_BARGAIN, bargainId, 0);
            case BARGAIN_RESULT_TYPE_BARGAIN_COMPLETE_FAIL -> questManager.queueEvent(

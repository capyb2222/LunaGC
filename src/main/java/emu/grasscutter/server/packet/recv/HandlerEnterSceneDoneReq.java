...
import emu.grasscutter.net.proto.EnterSceneDoneReq;
...
public class HandlerEnterSceneDoneReq extends PacketHandler {
    ...
    public void handle(GameSession session, byte[] header, byte[] payload) throws Exception {
        ...
        if (currentHpDebts > 0.0f) {
            avatarEntity.getWorld().broadcastPacket(new PacketEntityFightPropChangeReasonNotify(avatarEntity, FightProperty.FIGHT_PROP_CUR_HP_DEBTS, currentHpDebts, PropChangeReason.PROP_CHANGE_REASON_PROP_CHANGE_NONE, _ChangeHpDebtsReason._CHANGE_HP_DEBTS_REASON_CHANGE_HP_DEBTS_NONE));
        }
        // spawn NPC

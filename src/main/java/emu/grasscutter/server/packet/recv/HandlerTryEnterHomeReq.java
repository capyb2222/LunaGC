    ...
    public void handle(GameSession session, byte[] header, byte[] payload) throws Exception {
        ...
        if (req.getTargetUid() != session.getPlayer().getUid()) {
            ...
            switch (targetHome.getEnterHomeOption()) {
                case FriendEnterHomeOption
                        .FRIEND_ENTER_HOME_OPTION_NEED_CONFIRM_VALUE -> {
                    if (!targetPlayer.isOnline()) {
                        session.send(
                                new PacketTryEnterHomeRsp(
                                        Retcode.RET_HOME_OWNER_OFFLINE_VALUE, req.getTargetUid()));
                    } else {
                        session
                                .getServer()
                ...
                case FriendEnterHomeOption
                    ...
                    session.send(
                            new PacketTryEnterHomeRsp(
                                    Retcode.RET_HOME_HOME_REFUSE_GUEST_ENTER_VALUE,
                                    req.getTargetUid()));
                }

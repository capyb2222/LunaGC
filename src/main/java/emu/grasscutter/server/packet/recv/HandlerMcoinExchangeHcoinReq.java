...
public class HandlerMcoinExchangeHcoinReq extends PacketHandler {
    ...
    public void handle(GameSession session, byte[] header, byte[] payload) throws Exception {
        McoinExchangeHcoinReq exchangeReq =
                McoinExchangeHcoinReq.parseFrom(payload);

        if (session.getPlayer().getCrystals() < exchangeReq.getMcoinCost()
                && exchangeReq.getMcoinCost() == exchangeReq.getHcoin()) {
            session.send(
                    new PacketMcoinExchangeHcoinRsp(Retcode.RET_UNKNOWN_ERROR_VALUE));
            return;
        }
        ...
        session.send(new PacketMcoinExchangeHcoinRsp(Retcode.RET_SUCC_VALUE));
    }
}
